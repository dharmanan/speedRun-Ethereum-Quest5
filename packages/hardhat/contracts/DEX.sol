...
contract DEX {
    ...
    mapping(address => uint256) public liquidity; // Track liquidity per user (Liquidity Provider Tokens)
    ...
    function price(uint256 xInput, uint256 xReserves, uint256 yReserves) public pure returns (uint256 yOutput) {
        uint256 xInputWithFee = xInput * 997; // 0.3% fee (997/1000 = 99.7%)
    ...
    function withdraw(uint256 amount) public returns (uint256 ethAmount, uint256 tokenAmount) {
        ...
        require(liquidity[msg.sender] >= amount, "Insufficient liquidity to withdraw");
        uint256 ethReserves = address(this).balance;
        uint256 tokenReserves = token.balanceOf(address(this));
